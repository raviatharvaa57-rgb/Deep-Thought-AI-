    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="bg-light-bg dark:bg-dark-sidebar rounded-lg shadow-xl w-full max-w-md p-6 relative" onClick={e => e.stopPropagation()}>
                ...
                <h2 className="text-2xl font-bold mb-6 text-center text-light-text dark:text-dark-text">Profile Settings</h2>
                
                <form onSubmit={handleSubmit}>
                    <div className="flex flex-col items-center mb-6">
                        <div className="relative">
                            <img src={currentAvatar} alt="Avatar" className="w-24 h-24 rounded-full object-cover ring-2 ring-offset-2 ring-offset-light-bg dark:ring-offset-dark-sidebar ring-indigo-500" />
                            <button type="button" onClick={() => fileInputRef.current?.click()} className="absolute bottom-0 right-0 bg-indigo-600 text-white rounded-full p-2 hover:bg-indigo-700 transition-transform transform hover:scale-110" aria-label="Change avatar">
                                <CameraIcon className="w-5 h-5" />
                            </button>
                            <input type="file" ref={fileInputRef} onChange={handleAvatarChange} className="hidden" accept="image/*" />
                        </div>
                        <button type="button" onClick={handleRemoveAvatar} className="mt-3 text-sm text-indigo-600 dark:text-indigo-400 hover:underline">
                            Remove Photo
                        </button>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex flex-col sm:flex-row gap-4">
                            <div className="flex-1">
                                <label htmlFor="firstName" className="block text-sm font-medium text-light-secondary-text dark:text-dark-secondary-text mb-1">First Name</label>
                                <input
                                  id="firstName"
                                  type="text"
                                  value={firstName}
                            ...
                            <div className="flex-1">
                                <label htmlFor="lastName" className="block text-sm font-medium text-light-secondary-text dark:text-dark-secondary-text mb-1">Last Name</label>
                                <input
                                  id="lastName"
                                  type="text"
                                  value={lastName}
